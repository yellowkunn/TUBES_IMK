<div class="mt-2 bg-white hidden md:block text-smallContent dark:dark-mode">
    <div class="flex justify-between items-center px-8 lg:px-12 py-5">
        ...
        <div class="flex gap-8 lg:gap-24">
            <a href="{{ route('berandasiswa') }}">
                <p
                    class="dark:text-white dark:font-normal text-baseBlue text-center font-bold lg:text-[18px] leading-snug">
                    Fortunate<br>Education Center</p>
            </a>
        ...
        <div class="flex items-center gap-4">
            <div class="group" id="setTheme">
                <button onclick="changeToDark()" id="darkModeBtn"
                    class="bg-yellow-400/30 group-hover:bg-black p-1.5 px-2"><i
                        class="group-hover:text-white text-yellow-400 fa-regular fa-moon fa-xl text-greyIcon"></i></button>
                <button onclick="changeToLight()" id="lightModeBtn"
                    class="hidden group-hover:bg-slate-500/30 p-1.5 px-2"><i class="fa-solid fa-sun fa-lg"></i></button>
            </div>
            ...
            @if (Route::has('login'))
                @auth
                        ...
                        <div class="relative group">
                            @if ($notif->count() > 0)
                                <div class="bg-red-600 absolute top-0 right-0 rounded-full text-white text-[11px] px-1">
                                    {{ $notif->count() }}</div>
                            @endif <a href="{{ route('notifikasi') }}"><i
                    ...
                    <div class="pe-4 my-auto">
                        ...
                        <div id="dd-menu"
                            class="invisible grid grid-cols-1 divide-y bg-white dark:dark-mode mt-2 rounded-md drop-shadow-regularShadow absolute md:top-14 lg:top-16 md:right-10 lg:right-14">
                            ...
                            <form method="POST" action="{{ route('logout') }}">
                                @csrf
                                <button type="submit"
                                    class="w-full h-full flex items-center gap-2 py-2 px-4 text-black dark:bg-white dark:tect-black dark:rounded">
...
<div class="relative block md:hidden bg-white dark:dark-mode">
    <div class="flex justify-between items-center px-2 sm:px-6 md:px-8 py-5">
        <a href="{{ route('berandasiswa') }}">
            <p class="text-baseBlue text-center font-bold leading-snug ms-1.5 dark:text-white dark:font-normal">
                Fortunate<br>Education Center</p>
        </a>
    ...
    <div class="hidden md:hidden z-20 fixed top-0 backdrop-brightness-50 w-full h-screen" id="menuHamburger">
        <div class="bg-white dark:dark-mode w-screen flex flex-col gap-5 p-10">
            ...
            <div class="flex justify-end">
                <button type="button"><i class="fa-solid fa-xmark fa-lg text-slate-600 dark:text-white"
                        id="btnClose"></i></button>
            </div>
