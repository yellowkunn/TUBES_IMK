...
{
    $request->validate([
        'tingkat_kelas' => 'required|string|max:255|regex:/^[a-zA-Z0-9\s\-_]+$/',
        'harga' => 'required|numeric|min:0',
        'rentang' => 'required|string|max:255|regex:/^\d{1,3}-\d{1,3} tahun|\d{1,3} tahun ke atas$/',
        'fasilitas' => 'required|string|max:1000', // Sesuaikan ukuran maksimal sesuai kebutuhan
        'gambar' => 'nullable|file|mimes:jpg,jpeg,png,bmp|max:2048', // Maksimal 2 MB
        'jadwal_hari' => 'required|in:Senin,Selasa,Rabu,Kamis,Jumat,Sabtu,Minggu',
        'durasi' => 'required|string|max:255|regex:/^\d{1,3} (jam|hari)$/i' // Contoh: "3 jam" atau "2 hari"
    ]);
